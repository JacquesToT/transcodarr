...
generate_ssh_key() {
    ...
    if [[ -f "$key_file" ]]; then
        if gum confirm "SSH key already exists in output folder. Regenerate?"; then
            ...
            gum style --foreground 46 "✓ Using existing SSH key"
            return 0
        fi
    fi
    ...
    gum style --foreground 46 "✓ SSH key pair generated in output/rffmpeg/.ssh/"
}
...
# On Synology that's: /volume2/docker/jellyfin/rffmpeg/rffmpeg.yml
...
EOF
    ...
    gum style --foreground 46 "✓ rffmpeg.yml created in output/rffmpeg/"
}
...
services:
  jellyfin:
    ...
    environment:
      - PUID=1000          # ← Change to your user ID (run 'id' on Synology)
...
**What do they do?**
- \`DOCKER_MODS\` → Installs rffmpeg in the container (enables distributed transcoding)
- \`FFMPEG_PATH\` → Tells Jellyfin to use rffmpeg instead of regular ffmpeg
...
# Set permissions:
ssh YOUR_USER@${nas_ip} "chmod 600 /volume2/docker/jellyfin/rffmpeg/.ssh/id_rsa"
\`\`\`
...
EOF
    ...
    gum style --foreground 46 "✓ docker-compose.yml created in output/"
    gum style --foreground 46 "✓ EXISTING_JELLYFIN_CHANGES.md created (for existing setups)"
}
...
### Mac not responding
- Make sure Mac doesn't go to sleep: System Settings → Energy → Prevent sleep
...
\`\`\`bash
scp -r output/monitoring/* YOUR_USER@${nas_ip}:/volume2/docker/monitoring/
\`\`\`
...
3. Go to ⚙️ → Data sources → Add data source
4. Select "Prometheus"
...
EOF
    ...
    gum style --foreground 46 "✓ SETUP_INSTRUCTIONS.md created in output/"
}
...
show_summary() {
    ...
    echo ""
    gum style --foreground 212 "📁 Generated files are in: ${OUTPUT_DIR}/"
    echo ""
    gum style --foreground 252 "   output/"
    gum style --foreground 252 "   ├── docker-compose.yml              ← New Jellyfin install"
    gum style --foreground 252 "   ├── EXISTING_JELLYFIN_CHANGES.md    ← Already have Jellyfin? Read this!"
    gum style --foreground 252 "   ├── SETUP_INSTRUCTIONS.md           ← Full guide"
    gum style --foreground 252 "   ├── rffmpeg/"
    gum style --foreground 252 "   │   ├── rffmpeg.yml"
    gum style --foreground 252 "   │   └── .ssh/"
    gum style --foreground 252 "   │       ├── id_rsa"
    gum style --foreground 252 "   │       └── id_rsa.pub"
    gum style --foreground 252 "   └── monitoring/                     ← Optional"
    gum style --foreground 252 "       ├── docker-compose.yml"
    gum style --foreground 252 "       ├── prometheus.yml"
    gum style --foreground 252 "       └── grafana-dashboard.json"
...
# Copy this file to your server: /volume2/docker/monitoring/prometheus.yml
...
# Copy this file to your server: /volume2/docker/monitoring/docker-compose.yml
#
...
EOF
    ...
    gum style --foreground 46 "✓ Monitoring config created in output/monitoring/"
}
