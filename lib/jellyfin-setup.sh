...
create_rffmpeg_config() {
    ...
    # Always use jellyfin-ffmpeg (HDR/Dolby Vision support via tonemapx filter)
    local ffmpeg_path="/opt/jellyfin-ffmpeg/ffmpeg"
    local ffprobe_path="/opt/jellyfin-ffmpeg/ffprobe"

    mkdir -p "$(dirname "$config_file")"

    cat > "$config_file" << EOF
# Transcodarr - rffmpeg Configuration
# Using jellyfin-ffmpeg for HDR/Dolby Vision support
...
EOF
    ...
    show_result true "rffmpeg.yml created (jellyfin-ffmpeg)"
}
...
# DEPRECATED: Load balancer daemon removed - rffmpeg uses its own host selection
# Kept as no-op for backwards compatibility with existing calls
manage_load_balancer() {
    # No-op: load balancer daemon has been removed
    # Use ./monitor.sh to view node status instead
    return 0
}
